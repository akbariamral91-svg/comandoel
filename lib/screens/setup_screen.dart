...
class _SetupScreenState extends State<SetupScreen> {
  ...
  void _syncControllers() {
    ...
    while (_selectedCharacterIds.length < playerCount) {
      _selectedCharacterIds.add(null);
    }
    while (_selectedCharacterIds.length > playerCount) {
      _selectedCharacterIds.removeLast();
    }
  }

  Future<void> _pickCharacterFor(int index) async {
    final shop = context.read<ShopProvider>();
    final owned = shop.ownedCharacters;
        ...
        content: SizedBox(
          ...
          child: GridView.builder(
            ...
            itemBuilder: (context, i) {
              final c = owned[i];
              final isCurrent = _selectedCharacterIds[index] == c.id;
              return GestureDetector(
                ...
                child: Container(
                  decoration: BoxDecoration(
                    ...
                    border: Border.all(
                      color: isCurrent ? AppColors.brightGold : Colors.white24,
                      width: isCurrent ? 2.6 : 1.4,
                    ),
                    image: DecorationImage(
                      image: AssetImage(c.imagePath),
                      fit: BoxFit.cover,
                    ),
                  ),
                ),
    ...
    if (!mounted || selected == '__cancel__') return;
    setState(() {
      _selectedCharacterIds[index] = selected == '__none__' ? null : selected;
    });
  }
  ...
  void _startGame() async {
    ...
    final players = List.generate(playerCount, (i) {
      return Player(
        ...
        characterId: _selectedCharacterIds[i],
      );
    });
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          _PlayersPanel(
                            ...
                            onPickCharacter: _pickCharacterFor,
                          ),
                          SizedBox(height: isVeryNarrow ? 10 : 14),
                ...
                child: Align(
                  ...
                  child: Padding(
                    ...
                    child: GoldIconButton(
                      ...
                      size: (screenWidth * 0.11).clamp(48.0, 58.0),
...
class _PlayersPanel extends StatelessWidget {
  ...
  final ValueChanged<int> onPickCharacter;
              ...
              child: Row(
                ...
                children: [
                  Builder(builder: (context) {
                    final characterId = selectedCharacterIds[sourceIndex];
                    final character = Character.byId(characterId);
                    return GestureDetector(
                      onTap: () => onPickCharacter(sourceIndex),
                      child: CircleAvatar(
                        radius: avatarRadius + 3,
                        backgroundColor: Player.defaultAvatarColors[
                            sourceIndex % Player.defaultAvatarColors.length],
                        backgroundImage: character != null
                            ? AssetImage(character.imagePath)
                            : null,
                        child: character == null
                            ? Icon(Icons.add, size: avatarRadius, color: Colors.white70)
                            : null,
                      ),
                    );
                  }),
...
class _CounterBtn extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final icon = asset.contains('minus') ? Icons.remove_rounded : Icons.add_rounded;
    final btnSize = math.max(size, AppTheme.minTouchTarget);
    return Semantics(
      button: true,
      enabled: onTap != null,
      child: GestureDetector(
        ...
        child: Opacity(
          ...
          child: Container(
            ...
            child: Icon(icon, color: AppColors.gold, size: btnSize * 0.48),
          ),
        ),
      ),
    );
